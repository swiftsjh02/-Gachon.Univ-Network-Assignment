...
public class net {
	...
	public static void main(String args[]) {
        ...
        map.put("daum.net","6.7.8.8");
        map.put("nate.net","1.2.1.3");
		...
		try {
			...
			while(true){
				...
				ds.receive(dp);
				thread.execute(new dns_net(received,dp));
			}
			
		} catch(IOException e) {}
	}

    private static class dns_net implements Runnable{

        private DatagramPacket dp;
        private byte[] bf;
    
    
        dns_net(byte[] bf,DatagramPacket dp){
            this.bf=bf;
            this.dp=dp;
        }
